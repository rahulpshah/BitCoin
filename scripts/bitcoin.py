...
def computeDelta(wt, X, Xi):
  ...
  for i in xrange(len(Xivalues)):
    mean2 = np.mean(Xivalues[i])
    summation = 0.0
    temp = Xivalues[i]
    for j in xrange(len(Xvalues)):
      summation = summation + (Xvalues[j] - mean1) * (temp[j] - mean2)
    similarity = summation/(len(Xvalues) * np.std(Xvalues) * np.std(temp))
